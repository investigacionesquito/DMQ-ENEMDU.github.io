...
.libPaths("renv/library/R-4.3/x86_64-w64-mingw32")


# Instalar paquetes en caso que no esten ya instalados
if (!require(tidyverse) && !require(utils) && !require(purrr) && !require(kableExtra) && !require(DT) && !require(stringr)) {
  install.packages(c("tidyverse", "utils", "purrr", "kableExtra", "DT", "stringr"))
}


# Cargar los paquetes necesarios

packages <- c("tidyverse", "utils", "purrr", "kableExtra", "DT", "stringr")
...
DMQ_ciudad <- personas_2023 |> dplyr::filter(ciudad == "170150" & area == 1)
DMQ_urbano_2023 <- personas_2023 |> dplyr::filter(ciudad == "170150") 
DMQ_rural_2023 <- personas_2023 |> dplyr::filter(str_starts(ciudad, "1701") & ciudad != "170150") 
DMQ_2023 <- personas_2023 |> dplyr::filter(str_starts(ciudad, "1701")) 
...
resumen_poblacion_wide_ciudad <- resumen_poblacion_wide_ciudad |>
  left_join(total_etnia_ciudad, by = "etnia")|>
...
resumen_poblacion_wide_rural_DMQ <- resumen_poblacion_wide_rural_DMQ |>
  left_join(total_etnia_rural_DMQ, by = "etnia")|>
  select(etnia, total_personas, Porcentaje_personas, Cantidad_Mujeres, Porcentaje_Mujeres, Cantidad_Hombres, Porcentaje_Hombres) |> 
  arrange(desc(Porcentaje_personas))|> 
  na.omit() |>
  mutate(`Cómo se considera` = etnia,
...
resumen_poblacion_wide_DMQ <- resumen_poblacion_wide_DMQ |>
  left_join(total_etnia_DMQ, by = "etnia")|>
  select(etnia, total_personas, Porcentaje_personas, Cantidad_Mujeres, Porcentaje_Mujeres, Cantidad_Hombres, Porcentaje_Hombres) |> 
  arrange(desc(Porcentaje_personas))|> 
  na.omit() |>
  mutate(`Cómo se considera` = etnia,
...
#  Quito ciudad
resumen_poblacion_wide_ciudad <- kable(resumen_poblacion_wide_ciudad,
  ...
  row_spec(nrow(resumen_poblacion_wide_ciudad), bold = TRUE) |>
...
PET_ciudad <- DMQ_ciudad |>
  dplyr::filter(!is.na(condact) & condact %in% c(1:9)) |>
...
PEA_ciudad <- DMQ_ciudad |>
  dplyr::filter(!is.na(condact) & condact %in% c(1:8)) |>
  group_by(condact, p02) |>
  summarise(total = sum(fexp)) |>
  group_by(p02) |>
  summarise(total = sum(total)) |>
...
PEI_ciudad <- DMQ_ciudad |>
  dplyr::filter(!is.na(condact) & condact %in% c(9)) |>
  group_by(condact, p02) |>
  summarise(total = sum(fexp)) |>
  group_by(p02) |>
  summarise(total = sum(total)) |>
...
## agregar fila de poblacion total


fila_poblacion <- c("Población total", sum(CON_ciudad$Total), 0,sum(CON_ciudad$`Cantidad M`),0,sum(CON_ciudad$`Cantidad H`),0) 

names(fila_poblacion) <- names(agregado_ciudad)

fila_poblacion <- as.data.frame(t(fila_poblacion))

colnames(fila_poblacion) <- colnames(agregado_ciudad)

agregado_ciudad <- rbind(fila_poblacion, agregado_ciudad)

columnas_numericas <- colnames(agregado_ciudad)[-1]

agregado_ciudad[, columnas_numericas] <- sapply(agregado_ciudad[, columnas_numericas], as.numeric)
...
fila_poblacion <- c("Población total", sum(CON_urbano$Total), 0,sum(CON_urbano$`Cantidad M`),0,sum(CON_urbano$`Cantidad H`),0) 

names(fila_poblacion) <- names(agregado_urbano)

fila_poblacion <- as.data.frame(t(fila_poblacion))

colnames(fila_poblacion) <- colnames(agregado_urbano)

agregado_urbano <- rbind(fila_poblacion, agregado_urbano)

columnas_numericas <- colnames(agregado_urbano)[-1]

agregado_urbano[, columnas_numericas] <- sapply(agregado_urbano[, columnas_numericas], as.numeric)
...
fila_poblacion <- c("Población total", sum(CON_rural$Total), 0,sum(CON_rural$`Cantidad M`),0,sum(CON_rural$`Cantidad H`),0) 

names(fila_poblacion) <- names(agregado_rural)

fila_poblacion <- as.data.frame(t(fila_poblacion))

colnames(fila_poblacion) <- colnames(agregado_rural)

agregado_rural <- rbind(fila_poblacion, agregado_rural)

columnas_numericas <- colnames(agregado_rural)[-1]

agregado_rural[, columnas_numericas] <- sapply(agregado_rural[, columnas_numericas], as.numeric)
...
fila_poblacion <- c("Población total", sum(CON_DMQ$Total), 0,sum(CON_DMQ$`Cantidad M`),0,sum(CON_DMQ$`Cantidad H`),0) 

names(fila_poblacion) <- names(agregado_DMQ)

fila_poblacion <- as.data.frame(t(fila_poblacion))

colnames(fila_poblacion) <- colnames(agregado_DMQ)

agregado_DMQ <- rbind(fila_poblacion, agregado_DMQ)

columnas_numericas <- colnames(agregado_DMQ)[-1]

agregado_DMQ[, columnas_numericas] <- sapply(agregado_DMQ[, columnas_numericas], as.numeric)
